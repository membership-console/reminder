...
class DeleteNotificationUseCase_UT extends AbstractUseCase_UT {
    ...
    def "handle: 投稿者、もしくはリマインダーの管理者がお知らせを削除"() {
        ...
        1 * loginUser.hasRole(Role.REMINDER_ADMIN) >> expectedHasRole
        (0..1) * notification.isContributed(loginUser) >> expectedIsContributed
        1 * this.notificationRepository.deleteById(notification.id)

        where:
        expectedHasRole | expectedIsContributed
        true            | true
        true            | false
        false           | true
    }
