...
class NotificationReminderRestController_IT extends AbstractRestController_IT {
    ...
    def "リマインダー作成API: 正常系 投稿者、もしくはリマインダーの管理者がリマインダーを作成"() {
        ...
        final requestBody = new NotificationReminderCreateRequest(DateHelper.tomorrow().toLocalDate())
        ...
        requestBody.scheduledDate.isEqual((createdNotificationReminder.scheduled_date as Date).toLocalDate())
    ...
    def "リマインダー作成API: 異常系 リクエストボディのバリデーション"() {
        ...
        final requestBody = new NotificationReminderCreateRequest(inputScheduledDate)
        ...
        where:
        inputScheduledDate           || expectedErrorCode
        LocalDate.now().minusDays(1) || ErrorCode.INVALID_NOTIFICATION_REMINDER_DATE
    }
